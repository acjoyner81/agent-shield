...
3b. **Multica Task Sync.** After the spec is accepted and linked, check if the `multica` CLI tool is available (`which multica` / `~/.local/bin/multica`). If available, create a tracking issue for the feature:
    ```bash
    multica issue create --title "<Feature Title>" --description "<Spec Summary>" --parent "<Epic-ID-or-Parent>"
    ```
    Record the returned Multica issue identifier in the chat summary.
